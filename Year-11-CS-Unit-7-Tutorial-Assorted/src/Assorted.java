...
public class Assorted {
    ...
    public static int busStop(List<Integer[]> list) {
        ...
        for (Integer[] stop : list) {
            passengers += stop[0]; // add passengers getting on
            passengers -= stop[1]; // subtract passengers getting off
        }
        return passengers;
    }
    ...
    public static int toBinary(List<Integer> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            decimal += list.get(list.size() - i - 1) * Math.pow(2, i);
        }
        return decimal;
    }
